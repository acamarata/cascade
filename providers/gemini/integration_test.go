...
// Purpose: the tagged live lane against the real Gemini API (Art.2's
...
// Constraints: credentials via env-ref only (GEMINI_API_KEY); without it,
// the test reports an explicit skip reason and never silently passes as
// a real-counterpart proof.
...
func liveDriver(t *testing.T) *Driver {
	...
	if key == "" {
		t.Skip("skip: GEMINI_API_KEY is not set; this lane proves the real-counterpart claim and never substitutes a fake for it")
	}
	d, err := New(Config{
...
// TestGeminiDriverLiveAPI exercises Chat and Stream end-to-end against the
// real Gemini API.
func TestGeminiDriverLiveAPI(t *testing.T) {
	d := liveDriver(t)
