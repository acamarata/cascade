...
bus). This file currently documents the ONE subsystem that ticket
establishes — `internal/events`. Ticket contract:
`.claude/planning/p1/phase/epics/E-C/waves/W-1/sprints/S-04/tickets/T-3.yaml`.
Other subsystems (the scheduler, hooks, doctor, crash recovery, the daemon
composition root) are out of this ticket's scope and are not documented
here yet — each adds its own section as it lands.
