...
//   gains no new method, and every caller reaches "model.execute" and
//   "job.cancel" through this file's Client wrapper instead.
...
//   ModelExecute is exactly one Do(ctx, "model.execute", req, &resp) call.
//   job.cancel is an idempotent JSON-RPC method (R-21.264) - JobCancel
//   itself holds no dedupe state and simply forwards every call; a repeat
//   call is a no-op only because the daemon's own handler is idempotent,
//   not because this wrapper remembers anything. Cancellation is never
//   written to the SSE stream (GET /events is delivery only).
// SPORT: pkg.provider.client-wrapper/ADD (P1-E10-W3-S19-T1).
...
// modelExecuteMethod is the JSON-RPC method name ModelExecute dials.
const modelExecuteMethod = "model.execute"
...
// ModelExecute dispatches req to the daemon's model.execute door and
// returns its ModelResponse. It is exactly one
// Do(ctx, "model.execute", req, &resp) call; any error Do returns is mapped
// through the pkg/cascade taxonomy before it reaches the caller.
func (c *Client) ModelExecute(ctx context.Context, req ModelRequest) (ModelResponse, error) {
	var resp ModelResponse
	if err := c.caller.Do(ctx, modelExecuteMethod, req, &resp); err != nil {
		return ModelResponse{}, taxonomyError(err, "provider: model.execute failed")
	}
	return resp, nil
}
