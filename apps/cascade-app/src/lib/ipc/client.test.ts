...
describe('ipc.resolve', () => {
  ...
  it('passes undefined args when tier and format are omitted', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('cascade_resolve', { tier: undefined, format: undefined });
  });
});
...
describe('ipc.search', () => {
  it('calls invoke("cascade_search") with query and limit', async () => {
    ...
    expect(mockInvoke).toHaveBeenCalledWith('cascade_search', { query: 'delegation mandate', limit: 5 });
...
describe('ipc.inbox.send', () => {
  it('calls invoke("cascade_inbox_send") with to/subject/body/priority args', async () => {
    ...
    mockInvoke.mockRejectedValueOnce('NotImplemented: cascade_inbox_send — awaits T-P4-E01 daemon inbox_send method');
...
describe('CascadeIpcError wrapping', () => {
  it('parses structured JSON error payload', async () => {
    mockInvoke.mockRejectedValueOnce(JSON.stringify({ code: 'IpcTimeout', message: 'request timed out after 5000ms' }));
