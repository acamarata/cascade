...
fn accounts_path() -> PathBuf {
    cascade_types::paths::home_dir().join(".cascade").join("accounts.json")
}

fn quota_store_path() -> PathBuf {
    cascade_types::paths::home_dir().join(".cascade").join("quota-store.json")
}
...
fn quota_summary(account: &Account, store: &Option<QuotaStore>) -> String {
    let store = match store { Some(s) => s, None => return "—".to_string() };

    let entry = store.accounts.iter()
        .find(|e| account.quota_account_id.as_deref().map(|q| e.account_id == q).unwrap_or(false))
        .or_else(|| store.accounts.iter().find(|e| e.account_id == account.id));

    let entry = match entry { Some(e) => e, None => return "—".to_string() };

    if let Some(win) = entry.rate_windows.first() {
        let lim = win.limit.map(|l| format!("/{}", l / 1_000)).unwrap_or_default();
...
fn build_menu_content(data: &FleetData) -> (String, Vec<String>) {
    let registry = match &data.registry {
        Some(r) => r,
        None => return ("Fleet: no data".to_string(), vec!["accounts.json not found".to_string()]),
    };
    ...
    for family in &[
        AccountFamily::Claude, AccountFamily::Openai, AccountFamily::Google,
        AccountFamily::Opencode, AccountFamily::Gfp,
    ] {
        let members: Vec<_> = registry.accounts.iter().filter(|a| &a.family == family).collect();
...
#[cfg(target_os = "macos")]
fn run_widget() {
    use cascade_tray::{MacosTrayImpl, TrayHandle, TrayAction};
    ...
    let mut tray = match MacosTrayImpl::new(&title) {
        Ok(t)  => t,
        Err(e) => { eprintln!("cascade-fleet-widget: tray init failed: {e}"); std::process::exit(1); }
    };

    apply_menu(&mut tray, &title, &lines);
    if let Err(e) = tray.show() { eprintln!("cascade-fleet-widget: show failed: {e}"); }
    ...
    loop {
        while let Some(action) = tray.last_action() {
            match action {
                TrayAction::Quit => { println!("cascade-fleet-widget: quit"); std::process::exit(0); }
...
fn apply_menu(tray: &mut cascade_tray::MacosTrayImpl, title: &str, lines: &[String]) {
    use cascade_tray::{TrayHandle, TrayAction, TrayMenuItem, TrayMenuSpec};

    let mut items = vec![
        TrayMenuItem::Action { label: title.to_string(), action: TrayAction::OpenApp },
        TrayMenuItem::Separator,
    ];
    for line in lines {
        items.push(TrayMenuItem::Action { label: line.clone(), action: TrayAction::OpenApp });
    }
    items.push(TrayMenuItem::Separator);
    items.push(TrayMenuItem::Action { label: "Refresh now".to_string(), action: TrayAction::OpenDashboard });
    items.push(TrayMenuItem::Action { label: "Quit".to_string(), action: TrayAction::Quit });
...
fn push_state(tray: &mut cascade_tray::MacosTrayImpl, lines: &[String]) {
    ...
    let state = TrayState { fleet_summary: lines.to_vec(), ..TrayState::default() };
    if let Err(e) = tray.update(&state) { eprintln!("cascade-fleet-widget: update failed: {e}"); }
}
...
mod tests {
    ...
    fn no_data_returns_fallback_title() {
        let data = FleetData { registry: None, store: None };
        let (title, _) = build_menu_content(&data);
        assert!(title.contains("Fleet") || title.contains("no data"), "bad title: {title}");
    }
    ...
    fn quota_summary_no_store_is_dash() {
        use cascade_types::accounts::AccessMethod;
        let account = Account {
            id: "t".to_string(), family: AccountFamily::Claude,
            subscription: "s".to_string(), access_methods: vec![AccessMethod::NativeCc],
            role: AccountRole::PrimaryT0, exhaustion_priority: 255,
            models: vec![], cli_available: true, key_count: 0,
            quota_account_id: None, notes: None,
        };
    ...
    fn role_badges_are_distinct() {
        ...
        let badge_arr = [
            role_badge(&AccountRole::PrimaryT0), role_badge(&AccountRole::Pooled),
            role_badge(&AccountRole::Free),      role_badge(&AccountRole::Fallback),
        ];
    ...
    fn family_labels_non_empty() {
        for f in &[AccountFamily::Claude, AccountFamily::Openai, AccountFamily::Google,
                   AccountFamily::Opencode, AccountFamily::Gfp] {
