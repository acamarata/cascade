...
static int diskann_candidate_list_insert(
  ...
  // Check for duplicate. Callers use this same path to replace an approximate
  // quantized distance with the full-precision distance. The exact distance is
  // not guaranteed to be numerically smaller (for example, Hamming and cosine
  // distances have different scales), so always update and re-sort both ways.
  for (int i = 0; i < list->count; i++) {
    if (list->items[i].rowid == rowid) {
      list->items[i].distance = distance;
      struct Vec0DiskannCandidate tmp = list->items[i];
      if (i > 0 && list->items[i - 1].distance > tmp.distance) {
        int j = i - 1;
        ...
        while (j + 1 < list->count &&
               list->items[j + 1].distance < tmp.distance) {
