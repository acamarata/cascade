...
// Purpose: the install half of the cascade-opencode lifecycle — invoke the
...
//   internal/context's Opencode instruction writer (the E/S-09.T3 generator).
...
// SPORT: plugins/opencode install (ADD) — P1-E16-W4-S34-T3.
...
// GeneratorFunc produces the Opencode instruction golden for cwd. A real
...
func unwiredGenerator(context.Context, string) ([]GeneratedFile, error) {
	return nil, fmt.Errorf("cascade-opencode: instruction generator not wired (internal/plugins/registry.go must call opencode.SetGenerator)")
}
...
func SetGenerator(g GeneratorFunc) error {
	if g == nil {
		return fmt.Errorf("cascade-opencode: SetGenerator: generator must not be nil")
	}
	Generate = g
	return nil
}
...
// Install renders the Opencode instruction golden for cwd via Generate and
...
func writeIfChanged(path string, content []byte) (bool, error) {
	...
	case !os.IsNotExist(err):
		return false, fmt.Errorf("cascade-opencode: read %s: %w", path, err)
	}
...
func writeAtomic(path string, data []byte) error {
	...
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("cascade-opencode: create directory %s: %w", dir, err)
	}
	tmp, err := os.CreateTemp(dir, ".cascade-opencode-*")
	if err != nil {
		return fmt.Errorf("cascade-opencode: create temp file in %s: %w", dir, err)
	}
	tmpName := tmp.Name()
	if _, err := tmp.Write(data); err != nil {
		_ = tmp.Close()
		_ = os.Remove(tmpName)
		return fmt.Errorf("cascade-opencode: write %s: %w", path, err)
	}
	if err := tmp.Close(); err != nil {
		_ = os.Remove(tmpName)
		return fmt.Errorf("cascade-opencode: close %s: %w", path, err)
	}
	if err := os.Chmod(tmpName, 0o644); err != nil {
		_ = os.Remove(tmpName)
		return fmt.Errorf("cascade-opencode: set mode on %s: %w", path, err)
	}
	if err := os.Rename(tmpName, path); err != nil {
		_ = os.Remove(tmpName)
		return fmt.Errorf("cascade-opencode: replace %s: %w", path, err)
	}
	return nil
}
