...
// TestCascadeCodexUninstallHook proves the full install-then-uninstall
...
func TestCascadeCodexUninstallHook(t *testing.T) {
	dir := t.TempDir()
...
func TestUninstallKeepsAFileAnotherHarnessStillReads(t *testing.T) {
	...
	results, err := Uninstall(context.Background(), dir, SharedPaths{path: "opencode"})
	if err != nil {
		t.Fatalf("Uninstall: %v", err)
	}
