...
// TestCascadeCodexDetectPredicate exercises both the present and absent
// PATH states via the injectable lookPath variable, without touching the
// real filesystem or the developer's real PATH.
func TestCascadeCodexDetectPredicate(t *testing.T) {
	prev := lookPath
	...
	t.Run("present", func(t *testing.T) {
		lookPath = func(name string) (string, error) {
			if name != codexBinary {
				t.Fatalf("lookPath called with %q, want %q", name, codexBinary)
			}
...
func TestDefaultLookPathFindsRealExecutable(t *testing.T) {
	...
	dir := t.TempDir()
	target := filepath.Join(dir, "fake-codex-binary")
	...
	got, err := defaultLookPath("fake-codex-binary")
	if err != nil {
...
func TestCandidateNamesNonWindows(t *testing.T) {
	...
	got := candidateNames("codex")
	if len(got) != 1 || got[0] != "codex" {
		t.Errorf("candidateNames(codex) = %v, want [codex]", got)
	}
}
