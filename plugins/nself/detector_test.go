...
func TestDetect_MarkerHitSkipsTheProbeEntirely(t *testing.T) {
	...
	res := d.Detect(context.Background())
	if !res.Detected || res.Method != "marker-dir" || res.MarkerPath != filepath.FromSlash("/repo/.nself") || runner.calls != 0 {
...
func TestOsStatFS_RealFilesystemOutcomes(t *testing.T) {
	...
	if runtime.GOOS == "windows" {
		t.Skip("Windows stat of a path through a regular file does not reliably fail the same way; the portable checks above already ran")
	}
...
func TestProjectMarkerAt_UnreadableProjectFileIsReported(t *testing.T) {
	fs := newFakeFS()
	fs.dirs[filepath.FromSlash("/repo/.nself")] = true
