...
func TestScanAncestors_FindsRealBackendLayout(t *testing.T) {
	...
	if want := "/repo/backend/.nself"; got != want {
...
func TestScanAncestors_VCSRootItselfIsStillExamined(t *testing.T) {
	...
	fs.dirs["/srv/repo/.git"] = true
	if got, _ := scanAncestors(fs, "/srv/repo/apps", ""); got != "/srv/repo/.nself" {
...
func TestScanAncestors_ContinuesPastUnreadableAncestor(t *testing.T) {
	...
	fs.statErr["/a/b/.nself"] = syscall.EACCES
	got, err := scanAncestors(fs, "/a/b/c", "")
	if got != "/a/.nself" {
		t.Fatalf("scanAncestors = %q, want /a/.nself: the walk must continue past an unreadable ancestor", got)
	}
...
func TestScanAncestors_AllUnreadableReportsRatherThanPanics(t *testing.T) {
	fs := newFakeFS()
	for _, p := range []string{"/a/b/.nself", "/a/.nself", "/.nself"} {
