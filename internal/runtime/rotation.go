...
// path.1 (os.Rename — atomic on every platform this ticket targets), and
// opens a fresh active file carrying one JSON line naming the rotated
// path and its sequence number (always 1: the backup is, by
// construction, always the newest).
func (w *RotatingWriter) rotateLocked() error {
	...
	if err := os.Rename(w.path, rotated); err != nil {
		return &LogError{Field: "logging", Reason: fmt.Sprintf("rotate log file %s to %s: %v", w.path, rotated, err)}
	}

	f, err := os.OpenFile(w.path, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil {
		return &LogError{Field: "logging", Reason: fmt.Sprintf("open log file %s after rotation: %v", w.path, err)}
	}
	w.file = f
