...
func TestOAuthSingleFlightRefreshIssuesExactlyOneRequest(t *testing.T) {
	...
	// This was a bare `<-arrived`, which is a hang by construction: it
	// assumes the leader reaches the exchanger, and if the leader returns
	// EARLY for any reason -- a refusal, a validation error, a regression
	// in Refresh -- nothing is ever sent and the receive blocks forever.
	// The windows lane proved it: Refresh returns before the exchanger is
	// touched there, so this test consumed the whole 10-minute package
	// timeout and took every other test in internal/secrets down with it,
	// reporting a hang instead of the one-line refusal that actually
	// happened. Reading errs[0] after leaderDone is closed is race-free:
	// the close happens-after the write.
	select {
	case <-arrived:
	case <-leaderDone:
	...
	for i := 1; i < callers; i++ {
		...
		case <-joined:
		case <-time.After(gateArrivalTimeout):
			t.Fatalf("only %d of %d followers reached the single-flight gate within %s", i-1, callers-1, gateArrivalTimeout)
		}
	}
	close(release)
	wg.Wait()
